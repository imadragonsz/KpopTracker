    ...
    if (user.id === lastCheckedUserId && lastCheckedIsAdmin !== null) {
      ...
      if (uploadForm) {
        if (isAdmin) {
          ...
          console.log(
            `[photocardGallery] User: ${user.email}, isAdmin: ${isAdmin} (cached) -- upload form shown`
          );
        } else {
          uploadForm.classList.add("hidden-upload-form");
          uploadForm.style.display = "none";
          console.log(
            `[photocardGallery] User: ${user.email}, isAdmin: ${isAdmin} (cached) -- upload form hidden`
          );
        }
      }
    } else {
      ...
      if (uploadForm) {
        if (isAdmin) {
          ...
          uploadForm.style.display = "";
          console.log(
            `[photocardGallery] User: ${user.email}, isAdmin: ${isAdmin} -- upload form shown`
          );
        } else {
          uploadForm.classList.add("hidden-upload-form");
          uploadForm.style.display = "none";
          console.log(
            `[photocardGallery] User: ${user.email}, isAdmin: ${isAdmin} -- upload form hidden`
          );
        }
      }
    }
...
async function populateMembers(groupId, targetSelect, isFilter = false) {
  targetSelect.innerHTML = isFilter
...
async function renderGallery(photocards) {
  ...
  if (!photocards.length) {
    galleryContainer.innerHTML =
      '<div class="text-center text-muted">No photocards uploaded yet.</div>';
    return;
  }
  ...
  for (const card of pageCards) {
    ...
    if (card.groupId && card.memberId)
      memberName = await getMemberName(card.groupId, card.memberId);
    // Responsive image URLs
    const baseUrl = card.url.replace(/(-[0-9]+)?\.webp$/, "");
    const src400 = `${baseUrl}-400.webp`;
    const src800 = `${baseUrl}-800.webp`;
    const src1200 = `${baseUrl}-1200.webp`;
    item.innerHTML = `
      <img src="${src400}"
      ...
      <div class="photocard-actions">
        ...
        <button class="btn btn-sm btn-danger delete-photocard" data-filename="${
          card.filename
        }" style="display:${
      ...
      <div class="photocard-meta px-2 pb-2" style="font-size:0.95em;color:#bfc9db;">
        ${
          groupName
            ? `<span><i class='bi bi-people'></i> ${groupName}</span>`
            : ""
        }
        ${
          memberName
            ? `<span class='ms-2'><i class='bi bi-person'></i> ${memberName}</span>`
            : ""
        }
      </div>
    `;
...
function createLightbox() {
  ...
  if (!modal) {
    ...
    modal.innerHTML =
      '<img id="photocardLightboxImg" style="max-width:90vw;max-height:90vh;border-radius:18px;box-shadow:0 4px 32px #43c6ac55;outline:2px solid #43c6ac;">';
